...
old_sheets = "const SHEETS={CONFIG:'CONFIGURACOES',PARTICIPANTES:'PARTICIPANTES',OPERADORES:'OPERADORES',GRUPOS:'GRUPOS',JOGOS:'JOGOS',CLASSIFICACAO:'CLASSIFICACAO',MATA_MATA:'MATA_MATA',HISTORICO:'HISTORICO'};"
new_sheets = "const SHEETS={CONFIG:'CONFIGURACOES',PARTICIPANTES:'PARTICIPANTES',OPERADORES:'OPERADORES',GRUPOS:'GRUPOS',JOGOS:'JOGOS',CLASSIFICACAO:'CLASSIFICACAO',MATA_MATA:'MATA_MATA',HISTORICO:'HISTORICO',ARBITROS:'ARBITROS',CALENDARIO:'CALENDARIO'};"
if old_sheets in text:
    text = text.replace(old_sheets, new_sheets, 1)
...
a = text.index('function doGet(e){')
b = text.index('\nfunction doPost(e){', a)
text = text[:a] + new_get + text[b+1:]
a = text.index('function doPost(e){')
b = text.index('\nfunction adminLogin_(p){', a)
text = text[:a] + new_post + text[b+1:]

old_check = "  if(String(getConfig_('STATUS_INSCRICOES')||'').toUpperCase()!=='ABERTAS')return {ok:false,erro:'INSCRICOES_ENCERRADAS',mensagem:'As inscrições não estão abertas neste momento.'};"
new_check = "  const liberacao=validarInscricoesAbertas_();if(!liberacao.ok)return liberacao;"
if old_check in text:
    text = text.replace(old_check, new_check, 1)

fs = text.index('function salvarResultadoJogo_(d,usuario){')
fe = text.index('\nfunction montarResultado_', fs)
bloco = text[fs:fe]
if 'validarJogoPodeReceberResultado_' not in bloco:
    pos = bloco.find('\n', bloco.find('const id='))
    bloco = bloco[:pos+1] + "  const liberacaoAgenda=validarJogoPodeReceberResultado_(id);if(!liberacaoAgenda.ok)return liberacaoAgenda;\n" + bloco[pos+1:]
if 'TERCEIRO_LUGAR_PENDENTE' not in bloco:
    alvo = "  const r=sh.getRange(linha,1,1,h.length).getValues()[0],fase=String(r[idx.FASE]||''),categoria=String(r[idx.CATEGORIA]||''),statusAnterior=String(r[idx.STATUS]||''),melhorDe=melhorDePorFase_(fase);\n"
    guarda = "  if(fase==='FINAL'){const terceiro=listarJogos_().find(j=>j.categoria===categoria&&String(j.fase).toUpperCase()==='TERCEIRO_LUGAR');if(terceiro&&terceiro.status!=='FINALIZADO')return {ok:false,erro:'TERCEIRO_LUGAR_PENDENTE',mensagem:'A disputa de 3º lugar deve ser realizada antes da final.'};}\n"
    if alvo not in bloco:
        raise SystemExit('Linha-base da guarda da final não encontrada.')
    bloco = bloco.replace(alvo, alvo + guarda, 1)
text = text[:fs] + bloco + text[fe:]
code_path.write_text(text, encoding='utf-8')

arb_path = Path('backend/ARBITRAGEM_AGENDA.gs')
arb = arb_path.read_text(encoding='utf-8')
old = "  const jogos=lerJogosArbitragem_().filter(j=>j.status!=='FINALIZADO'&&j.data),arbitros=listarArbitros_().filter(a=>a.status==='ATIVO');"
new = "  const jogos=lerJogosArbitragem_().filter(j=>j.status!=='FINALIZADO'&&j.status!=='CANCELADA_PARA_REMARCACAO'&&j.data&&j.horario&&j.mesa),arbitros=listarArbitros_().filter(a=>a.status==='ATIVO');"
if old in arb:
    arb = arb.replace(old, new, 1)
    arb = arb.replace("if(!jogos.length)return {ok:false,erro:'SEM_JOGOS_AGENDADOS',mensagem:'Não existem partidas pendentes com data definida.'};", "if(!jogos.length)return {ok:false,erro:'SEM_JOGOS_AGENDADOS',mensagem:'Não existem partidas pendentes com data, horário e mesa completamente definidos.'};", 1)
arb_path.write_text(arb, encoding='utf-8')

enc_path = Path('backend/ENCERRAMENTO_TORNEIO.gs')
enc = enc_path.read_text(encoding='utf-8')
if 'arquivarExtrasOperacionais_(ss,pasta)' not in enc:
    enc = enc.replace("  criarPdfAba_(ss,'HISTORICO',pasta,'08_Historico.pdf');", "  criarPdfAba_(ss,'HISTORICO',pasta,'08_Historico.pdf');\n  if(typeof arquivarExtrasOperacionais_==='function')arquivarExtrasOperacionais_(ss,pasta);", 1)
if 'limparExtrasNovaEdicao_()' not in enc:
    enc = enc.replace("  limparAbaMantendoCabecalho_('HISTORICO');", "  limparAbaMantendoCabecalho_('HISTORICO');\n  if(typeof limparExtrasNovaEdicao_==='function')limparExtrasNovaEdicao_();", 1)
enc = enc.replace("  return ['PARTICIPANTES','GRUPOS','JOGOS','CLASSIFICACAO','MATA_MATA'].some", "  return ['PARTICIPANTES','GRUPOS','JOGOS','CLASSIFICACAO','MATA_MATA','ARBITROS','CALENDARIO'].some", 1)
if 'fingerprintExtrasOperacionais_' not in enc:
    enc = enc.replace("  return hashTexto_(partes.join('\\n'));", "  const extras=typeof fingerprintExtrasOperacionais_==='function'?fingerprintExtrasOperacionais_():'';\n  return hashTexto_(partes.join('\\n')+'\\n'+extras);", 1)
if "DATA_HORA_ENCERRAMENTO_INSCRICOES" not in enc:
    enc = enc.replace("  setConfig_('DATA_FINAL_PREVISTA','A DEFINIR','Data final prevista');", "  setConfig_('DATA_FINAL_PREVISTA','A DEFINIR','Data final prevista');\n  setConfig_('DATA_HORA_ENCERRAMENTO_INSCRICOES','','Data e hora limite das inscrições');\n  setConfig_('DATA_SORTEIO_GRUPOS','','Data pública prevista para o sorteio dos grupos');", 1)
enc_path.write_text(enc, encoding='utf-8')

print('Backend consolidado com sucesso.')
