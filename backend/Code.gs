...
const SHEETS={CONFIG:'CONFIGURACOES',PARTICIPANTES:'PARTICIPANTES',OPERADORES:'OPERADORES',GRUPOS:'GRUPOS',JOGOS:'JOGOS',CLASSIFICACAO:'CLASSIFICACAO',HISTORICO:'HISTORICO'};
...
function doGet(e){
  try{
    ...
    if(a==='torneioPublico')return jsonResponse_({ok:true,jogos:listarJogos_(),classificacao:listarClassificacao_(),grupos:listarGruposPublicos_()});
    ...
    if(a==='jogosAdmin'){const s=exigirSessao_(e.parameter.token);return jsonResponse_({ok:true,usuario:s.usuario,jogos:listarJogos_(),classificacao:listarClassificacao_(),grupos:listarGruposPublicos_()});}
...
function salvarResultadoJogo_(d,usuario){
  ...
  const sh=getSheet_(SHEETS.JOGOS),h=getHeaders_(sh),idx=indexHeaders_(h),linha=localizarLinhaPorValor_(sh,idx.ID_JOGO+1,id);if(linha===-1)return {ok:false,erro:'JOGO_NAO_ENCONTRADO',mensagem:'Partida não encontrada.'};const r=sh.getRange(linha,1,1,h.length).getValues()[0],fase=String(r[idx.FASE]||''),statusAnterior=String(r[idx.STATUS]||''),melhorDe=melhorDePorFase_(fase);
  if(statusAnterior==='FINALIZADO'&&fase!=='GRUPOS'&&usuario.nivel!=='ADMINISTRADOR')return {ok:false,erro:'CORRECAO_RESTRITA',mensagem:'Após a fase de grupos, somente o administrador pode corrigir uma partida finalizada.'};
  ...
  recalcularClassificacaoCategoria_(String(r[idx.CATEGORIA]));
  registrarHistorico_({usuario:usuario.email,perfil:usuario.nivel,acao:statusAnterior==='FINALIZADO'?'RESULTADO_CORRIGIDO':'RESULTADO_LANCADO',entidade:'JOGOS',idRegistro:id,valorAnterior:JSON.stringify(anterior),valorNovo:JSON.stringify(campos),observacoes:statusAnterior==='FINALIZADO'?'Resultado corrigido e classificação recalculada.':'Resultado finalizado e classificação recalculada.'});
  return {ok:true,mensagem:statusAnterior==='FINALIZADO'?'Resultado corrigido. A classificação foi recalculada.':'Partida finalizada. A classificação foi atualizada.',classificacao:listarClassificacao_().filter(x=>x.categoria===String(r[idx.CATEGORIA])&&x.grupo===String(r[idx.GRUPO]))};
}
function montarResultado_(d,tipo,melhorDe,j){
  const alvo=melhorDe===5?3:2,sets=[['',''],['',''],['',''],['',''],['','']];
  if(tipo==='WO_DUPLO')return {sets,setsA:0,setsB:0,pontosA:0,pontosB:0,idVencedor:'',vencedor:''};
  if(tipo==='WO_A'||tipo==='WO_B'){const aVence=tipo==='WO_B';for(let i=0;i<alvo;i++)sets[i]=aVence?[11,0]:[0,11];return {sets,setsA:aVence?alvo:0,setsB:aVence?0:alvo,pontosA:aVence?11*alvo:0,pontosB:aVence?0:11*alvo,idVencedor:aVence?j.idA:j.idB,vencedor:aVence?j.nomeA:j.nomeB};}
  let va=0,vb=0,pa=0,pb=0,encerrou=false;
  for(let i=0;i<melhorDe;i++){
    const sa=String(d['set'+(i+1)+'A']??'').trim(),sb=String(d['set'+(i+1)+'B']??'').trim();if(sa===''&&sb==='')continue;if(encerrou)throw new Error('SETS_EXCEDENTES|Não informe sets depois que um jogador já alcançou a quantidade necessária para vencer.');if(sa===''||sb===''||isNaN(Number(sa))||isNaN(Number(sb)))throw new Error('PLACAR_INVALIDO|Informe os dois pontos de cada set disputado.');const a=Number(sa),b=Number(sb);if(a<0||b<0||a===b||Math.max(a,b)<11||Math.abs(a-b)<2)throw new Error('SET_INVALIDO|Cada set termina com no mínimo 11 pontos e diferença mínima de 2.');sets[i]=[a,b];pa+=a;pb+=b;if(a>b)va++;else vb++;if(va===alvo||vb===alvo)encerrou=true;
  }
  if(va!==alvo&&vb!==alvo)throw new Error('PARTIDA_INCOMPLETA|Informe os sets necessários até um jogador vencer a partida.');return {sets,setsA:va,setsB:vb,pontosA:pa,pontosB:pb,idVencedor:va>vb?j.idA:j.idB,vencedor:va>vb?j.nomeA:j.nomeB};
}
...
function recalcularClassificacaoCategoria_(categoria){
  limparLinhasPorCategoria_(SHEETS.CLASSIFICACAO,categoria,'CATEGORIA');const membros=obterLinhasGrupo_(categoria),jogos=listarJogos_().filter(j=>j.categoria===categoria&&j.fase==='GRUPOS'&&j.status==='FINALIZADO');const porGrupo={};
  ...
  Object.keys(porGrupo).sort().forEach(grupo=>{const lista=Object.values(porGrupo[grupo]);lista.sort((a,b)=>b.vitorias-a.vitorias||((b.setsPro-b.setsContra)-(a.setsPro-a.setsContra))||((b.pontosPro-b.pontosContra)-(a.pontosPro-a.pontosContra))||b.pontosPro-a.pontosPro||String(a.nome).localeCompare(String(b.nome),'pt-BR'));lista.forEach((x,i)=>{const saldoSets=x.setsPro-x.setsContra,saldoPontos=x.pontosPro-x.pontosContra,aprov=x.jogos?x.vitorias/x.jogos:0;gravarObjetoPrimeiraLinhaLivre_(SHEETS.CLASSIFICACAO,'CATEGORIA',{CATEGORIA:categoria,GRUPO:grupo,ID_PARTICIPANTE:x.id,NOME:x.nome,JOGOS:x.jogos,VITORIAS:x.vitorias,DERROTAS:x.derrotas,SETS_PRO:x.setsPro,SETS_CONTRA:x.setsContra,SALDO_SETS:saldoSets,PONTOS_PRO:x.pontosPro,PONTOS_CONTRA:x.pontosContra,SALDO_PONTOS:saldoPontos,APROVEITAMENTO:aprov,POSICAO:i+1,CLASSIFICADO:i<2,CRITERIO_DESEMPATE:'Vitórias > saldo de sets > saldo de pontos > pontos pró'});});});
}
