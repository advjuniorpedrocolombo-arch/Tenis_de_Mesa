...
const SHEETS={CONFIG:'CONFIGURACOES',PARTICIPANTES:'PARTICIPANTES',OPERADORES:'OPERADORES',GRUPOS:'GRUPOS',JOGOS:'JOGOS',HISTORICO:'HISTORICO'};
...
function registrarInscricao_(d){
  ...
  const cats=categoriasPermitidas_();if(!cats.includes(categoria))return {ok:false,erro:'CATEGORIA_INVALIDA',mensagem:'Selecione uma categoria válida.'};
...
function obterEstadoSorteio_(categoria){
  ...
  const participantes=obterParticipantesValidos_().filter(p=>p.ativo&&p.statusInscricao==='APROVADO'&&(p.categoriaValidada||p.categoriaEscolhida)===categoria);
  const linhas=obterLinhasGrupo_(categoria),ids=new Set(linhas.map(x=>x.idParticipante));
  const grupos={};linhas.forEach(x=>{if(!grupos[x.nomeGrupo])grupos[x.nomeGrupo]={idGrupo:x.idGrupo,nome:x.nomeGrupo,ordem:x.ordem,status:x.status,participantes:[]};grupos[x.nomeGrupo].participantes.push(x)});
...
function iniciarSorteio_(d,usuario){
  const categoria=limparTexto_(d.categoria);if(!categoriasPermitidas_().includes(categoria))return {ok:false,erro:'CATEGORIA_INVALIDA',mensagem:'Categoria inválida.'};
  if(obterLinhasGrupo_(categoria).length)return {ok:false,erro:'SORTEIO_EXISTENTE',mensagem:'O sorteio desta categoria já foi iniciado.'};
  ...
  registrarHistorico_({usuario:usuario.email,perfil:usuario.nivel,acao:'SORTEIO_INICIADO',entidade:'GRUPOS',idRegistro:categoria,valorAnterior:'',valorNovo:JSON.stringify({grupos:cabecas.length}),observacoes:'Cabeças de chave posicionados automaticamente, um por grupo.'});
  return {ok:true,mensagem:'Sorteio iniciado. Os cabeças de chave já foram posicionados nos grupos.'};
}

function adicionarAoGrupo_(d,usuario){
  const categoria=limparTexto_(d.categoria),id=limparTexto_(d.idParticipante),nomeGrupo=limparTexto_(d.nomeGrupo);const estado=obterEstadoSorteio_(categoria);if(estado.status!=='EM_SORTEIO')return {ok:false,erro:'SORTEIO_NAO_ABERTO',mensagem:'O sorteio desta categoria não está aberto.'};
  const p=estado.naoSorteados.find(x=>x.id===id);if(!p)return {ok:false,erro:'PARTICIPANTE_INDISPONIVEL',mensagem:'Participante não disponível para sorteio.'};
  const g=estado.grupos.find(x=>x.nome===nomeGrupo);if(!g)return {ok:false,erro:'GRUPO_INVALIDO',mensagem:'Grupo inválido.'};
  const pos=obterLinhasGrupo_(categoria).filter(x=>!x.cabecaDeChave).length+1;
  gravarObjetoPrimeiraLinhaLivre_(SHEETS.GRUPOS,'ID_GRUPO',{ID_GRUPO:g.idGrupo,CATEGORIA:categoria,NOME_GRUPO:g.nome,ORDEM:g.ordem,ID_PARTICIPANTE:p.id,NOME_PARTICIPANTE:p.nome,POSICAO_SORTEIO:pos,CABECA_DE_CHAVE:false,DATA_GERACAO:new Date(),STATUS:'EM_SORTEIO'});
  registrarHistorico_({usuario:usuario.email,perfil:usuario.nivel,acao:'PARTICIPANTE_SORTEADO',entidade:'GRUPOS',idRegistro:p.id,valorAnterior:'',valorNovo:g.nome,observacoes:'Participante lançado durante sorteio ao vivo.'});
  return {ok:true,mensagem:p.nome+' inserido no '+g.nome+'.'};
}

function removerDoGrupo_(d,usuario){
  const categoria=limparTexto_(d.categoria),id=limparTexto_(d.idParticipante);const estado=obterEstadoSorteio_(categoria);if(estado.status!=='EM_SORTEIO')return {ok:false,erro:'SORTEIO_NAO_ABERTO',mensagem:'O sorteio não está aberto.'};
  const sh=getSheet_(SHEETS.GRUPOS),idx=indexHeaders_(getHeaders_(sh)),max=sh.getMaxRows();const vals=sh.getRange(2,1,max-1,sh.getLastColumn()).getValues();let linha=-1,row=null;for(let i=0;i<vals.length;i++){if(String(vals[i][idx.CATEGORIA])===categoria&&String(vals[i][idx.ID_PARTICIPANTE])===id){linha=i+2;row=vals[i];break}}
  if(linha===-1)return {ok:false,erro:'NAO_ENCONTRADO',mensagem:'Participante não encontrado nos grupos.'};if(row[idx.CABECA_DE_CHAVE]===true||String(row[idx.CABECA_DE_CHAVE]).toUpperCase()==='TRUE')return {ok:false,erro:'CABECA_FIXO',mensagem:'Cabeças de chave não podem ser removidos durante o sorteio.'};
  sh.getRange(linha,1,1,sh.getLastColumn()).clearContent();registrarHistorico_({usuario:usuario.email,perfil:usuario.nivel,acao:'SORTEIO_CORRIGIDO',entidade:'GRUPOS',idRegistro:id,valorAnterior:String(row[idx.NOME_GRUPO]),valorNovo:'REMOVIDO',observacoes:'Correção durante sorteio ao vivo.'});return {ok:true,mensagem:'Participante devolvido à lista de não sorteados.'};
}
...
function finalizarSorteio_(d,usuario){
  ...
  registrarHistorico_({usuario:usuario.email,perfil:usuario.nivel,acao:'SORTEIO_FINALIZADO',entidade:'GRUPOS',idRegistro:categoria,valorAnterior:'EM_SORTEIO',valorNovo:'FINALIZADO',observacoes:'Grupos bloqueados e confrontos gerados automaticamente.'});
  return {ok:true,mensagem:'Sorteio finalizado. Os grupos foram bloqueados e os confrontos foram gerados.'};
}

function reiniciarSorteio_(d,usuario){
  const categoria=limparTexto_(d.categoria),estado=obterEstadoSorteio_(categoria);if(estado.status==='FINALIZADO')return {ok:false,erro:'SORTEIO_FINALIZADO',mensagem:'O sorteio já foi finalizado. Não é permitido reiniciar após a geração dos confrontos.'};
  limparLinhasPorCategoria_(SHEETS.GRUPOS,categoria,'CATEGORIA');registrarHistorico_({usuario:usuario.email,perfil:usuario.nivel,acao:'SORTEIO_REINICIADO',entidade:'GRUPOS',idRegistro:categoria,valorAnterior:'EM_SORTEIO',valorNovo:'NAO_INICIADO',observacoes:'Sorteio apagado antes da finalização.'});return {ok:true,mensagem:'Sorteio reiniciado. Os cabeças de chave continuam definidos nos participantes.'};
}
...
function obterJogosCategoria_(categoria){const sh=getSheet_(SHEETS.JOGOS),headers=getHeaders_(sh),idx=indexHeaders_(headers),last=sh.getLastRow();if(last<2)return [];return sh.getRange(2,1,last-1,headers.length).getValues().filter(r=>String(r[idx.CATEGORIA])===categoria&&String(r[idx.FASE])==='GRUPOS'&&String(r[idx.ID_JOGO]||'').trim())}
...
function obterLinhasGrupo_(categoria){const sh=getSheet_(SHEETS.GRUPOS),headers=getHeaders_(sh),idx=indexHeaders_(headers),last=sh.getLastRow();if(last<2)return [];return sh.getRange(2,1,last-1,headers.length).getValues().filter(r=>String(r[idx.CATEGORIA])===categoria&&String(r[idx.ID_PARTICIPANTE]||'').trim()).map(r=>({idGrupo:r[idx.ID_GRUPO],categoria:r[idx.CATEGORIA],nomeGrupo:r[idx.NOME_GRUPO],ordem:Number(r[idx.ORDEM]),idParticipante:r[idx.ID_PARTICIPANTE],nomeParticipante:r[idx.NOME_PARTICIPANTE],posicaoSorteio:Number(r[idx.POSICAO_SORTEIO]||0),cabecaDeChave:r[idx.CABECA_DE_CHAVE]===true||String(r[idx.CABECA_DE_CHAVE]).toUpperCase()==='TRUE',status:String(r[idx.STATUS]||'')}))}
function limparLinhasPorCategoria_(aba,categoria,coluna){const sh=getSheet_(aba),headers=getHeaders_(sh),idx=indexHeaders_(headers),last=sh.getLastRow();if(last<2)return;const vals=sh.getRange(2,1,last-1,headers.length).getValues();for(let i=vals.length-1;i>=0;i--){if(String(vals[i][idx[coluna]])===categoria)sh.getRange(i+2,1,1,headers.length).clearContent()}}
...
function gravarObjetoPrimeiraLinhaLivre_(aba,colunaId,obj){const sh=getSheet_(aba),headers=getHeaders_(sh),idIndex=headers.indexOf(colunaId);if(idIndex===-1)throw new Error('Coluna não encontrada: '+colunaId);const linha=primeiraLinhaLivrePorColuna_(sh,idIndex+1,2),row=headers.map(h=>Object.prototype.hasOwnProperty.call(obj,h)?obj[h]:'');sh.getRange(linha,1,1,row.length).setValues([row])}
...
function indexHeaders_(headers){const idx={};headers.forEach((h,i)=>idx[String(h).trim()]=i);return idx}
